...
func (tq *TrackQuery) QueryUser() *UserQuery {
...
func (tq *TrackQuery) QueryCampaign() *CampaignQuery {
...
func (tq *TrackQuery) QueryLink() *CampaignLinkQuery {
	query := (&CampaignLinkClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TrackQuery) FirstIDX(ctx context.Context) int64 {
...
func (tq *TrackQuery) OnlyIDX(ctx context.Context) int64 {
...
func (tq *TrackQuery) IDs(ctx context.Context) (ids []int64, err error) {
...
func (tq *TrackQuery) WithUser(opts ...func(*UserQuery)) *TrackQuery {
	query := (&UserClient{config: tq.config}).Query()
...
func (tq *TrackQuery) WithCampaign(opts ...func(*CampaignQuery)) *TrackQuery {
	query := (&CampaignClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TrackQuery) WithLink(opts ...func(*CampaignLinkQuery)) *TrackQuery {
	query := (&CampaignLinkClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TrackQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Track, error) {
	...
	if tq.withUser != nil || tq.withCampaign != nil || tq.withLink != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withUser; query != nil {
		if err := tq.loadUser(ctx, query, nodes, nil,
			func(n *Track, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withCampaign; query != nil {
		if err := tq.loadCampaign(ctx, query, nodes, nil,
			func(n *Track, e *Campaign) { n.Edges.Campaign = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withLink; query != nil {
		if err := tq.loadLink(ctx, query, nodes, nil,
			func(n *Track, e *CampaignLink) { n.Edges.Link = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TrackQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Track, init func(*Track), assign func(*Track, *User)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Track)
	for i := range nodes {
		if nodes[i].user_tracks == nil {
			continue
		}
		fk := *nodes[i].user_tracks
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_tracks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TrackQuery) loadCampaign(ctx context.Context, query *CampaignQuery, nodes []*Track, init func(*Track), assign func(*Track, *Campaign)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Track)
	for i := range nodes {
		if nodes[i].campaign_tracks == nil {
			continue
		}
		fk := *nodes[i].campaign_tracks
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "campaign_tracks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TrackQuery) loadLink(ctx context.Context, query *CampaignLinkQuery, nodes []*Track, init func(*Track), assign func(*Track, *CampaignLink)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Track)
	for i := range nodes {
		if nodes[i].campaign_link_tracks == nil {
			continue
		}
		fk := *nodes[i].campaign_link_tracks
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(campaignlink.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "campaign_link_tracks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TrackQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(track.Table, track.Columns, sqlgraph.NewFieldSpec(track.FieldID, field.TypeInt64))
	_spec.From = tq.sql
